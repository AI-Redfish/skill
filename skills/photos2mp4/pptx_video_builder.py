...
#
# [tool.uv]
# # 首选运行方式：uv run pptx_video_builder.py ...（自动解析上述依赖并缓存，无需建 venv）
# # 说明：pywin32 仅 Windows 安装（export / list-transitions 实测需要）；
# #       mutagen 供 --auto-fit-music 读取音乐时长。PEP 723 块是纯注释，
# #       已装齐依赖的环境也可直接用 python 运行本脚本。
# ///
"""
...
依赖（已声明在本文件头部的 PEP 723 内联元数据中）：
  首选 uv：  uv run pptx_video_builder.py ...   # uv 自动解析并缓存依赖，无需建 venv
  直接 python：需已装齐 python-pptx + pywin32(仅 Windows，export/list-transitions
              实测需要) + mutagen(--auto-fit-music 需要)
...
# --------------------------------------------------------------------------
# 输出位置：默认产物目录（桌面固定文件夹）+ 本 skill 所在仓库防污染守卫
# --------------------------------------------------------------------------

_OUTPUT_DIRNAME = "photos2mp4_output"


def _desktop_dir():
    """当前用户桌面目录；Windows 优先用已知文件夹 API（识别 OneDrive 重定向），
    其余平台用 ~/Desktop；无法确定时返回 None。"""
    if os.name == "nt":
        try:
            import ctypes
            import uuid
            ...
            _u = uuid.UUID("B4BFCC3A-DB2C-424C-B029-7FE99A87C641")
            _fid = _KNOWNFOLDERID(_u.time_low, _u.time_mid, _u.time_hi_version,
...
def default_output_root():
    """产物默认根目录。优先级：环境变量 PHOTOS2MP4_OUT_DIR >
    <桌面>/photos2mp4_output > ~/photos2mp4_output。"""
    env = os.environ.get("PHOTOS2MP4_OUT_DIR")
    if env:
        return os.path.abspath(env)
    base = _desktop_dir() or os.path.expanduser("~")
    return os.path.join(base, _OUTPUT_DIRNAME)


def default_output_path(ext, prefix="photos"):
    """在默认产物目录下生成带时间戳的文件名（并确保目录存在）。"""
    root = default_output_root()
    _ensure_parent(os.path.join(root, "x"))
    return os.path.join(root, "%s_%s.%s" % (prefix, time.strftime("%Y%m%d_%H%M%S"), ext))


def _skill_repo_root():
    """本脚本所在 git 仓库根（向上找 .git）；不在仓库时为脚本所在目录。"""
...
def guard_output_path(path, what="产物"):
    """禁止把生成产物写进本 skill 所在 git 仓库（防止污染 git 工作区）。
    需要显式放行时设置环境变量 PHOTOS2MP4_ALLOW_REPO_OUTPUT=1。"""
    if os.environ.get("PHOTOS2MP4_ALLOW_REPO_OUTPUT") == "1":
        return path
    root = _skill_repo_root()
    if _is_under(path, root):
        raise SkillError(
            f"拒绝把{what}写入本 skill 所在仓库目录，以免污染 git 工作区：{path}\n"
            f"请输出到仓库之外（默认目录：{default_output_root()}）；"
            f"确有必要时设置 PHOTOS2MP4_ALLOW_REPO_OUTPUT=1 放行。")
    return path
...
def set_advance_only(slide, advance_ms):
    """只更新自动换片时间，保留已有切换效果；无切换元素时以 none 承载。"""
    sld = slide._element
    trs = []
    for child in sld:
        if child.tag == TAG_TRANSITION:
            trs.append(child)
        elif child.tag == TAG_AC:
            trs.extend(child.findall(".//" + TAG_TRANSITION))
    if trs:
        for tr in trs:  # AlternateContent 里 Choice/Fallback 各有一份，同步更新
            if advance_ms is None:
                tr.attrib.pop("advTm", None)
            else:
                tr.set("advTm", str(int(advance_ms)))
        return
    set_slide_transition(slide, {"transition": "none"}, advance_ms)
    ...
    for item in per_slide:
        ...
        for k in ("transition", "direction", "duration", "advance", "options"):
            if item.get(k) is not None:
                specs[idx][k] = item[k]
        # JSON 显式指定了切换效果但未指定方向时，不继承全局方向：
        # 各效果的方向取值互不兼容（如 wipe 的 up 对 fade 无意义）
        if item.get("transition") is not None and item.get("direction") is None:
            specs[idx]["direction"] = None
    if music and auto_fit_music:
...
def verify_transitions(workdir=None):
    ...
    try:
        ...
        # 版本门控：AC 包裹的未知 XML 会被旧版 PowerPoint 原样保留，
        # 探针会误报"支持"；放映时实际走 Fallback(fade)。
        # 因此低于要求版本的档次直接标记不支持，并跳过无意义的探测。
        tier_min_major = {"classic": 0, "p14": 14, "morph": 16}
        tier_ver_label = {"p14": "2010", "morph": "2019/365"}

        def _gated(tier):
            return ver_major < tier_min_major.get(tier, 0)

        def _gated_note(tier):
            return f"requires_powerpoint_{tier_ver_label.get(tier, '?')}+"
        ...
        # 需要真实探测的项（门控项跳过，但结果仍按目录原序输出）
        probe_entries = [(t, k) for t, k in entries if not _gated(t)]
        if not probe_entries:
            results = [_result(t, k, None, note=_gated_note(t))
                       for t, k in entries]
            return {"powerpoint_version": version, "results": results}
        probe_map = {tk: None for tk in probe_entries}
        probe_pptx = os.path.join(workdir, "probe.pptx")
        saved_pptx = os.path.join(workdir, "probe_saved.pptx")
        build_pptx([probe_png] * len(probe_entries), probe_pptx,
                   transition=probe_entries[0][1], advance=1.0,
                   per_slide=[{"slide": i + 1, "transition": key, "advance": 1.0}
                              for i, (_tier, key) in enumerate(probe_entries)])
        try:
            _open_save_close(os.path.abspath(probe_pptx), os.path.abspath(saved_pptx))
            prs = Presentation(saved_pptx)
            for i, slide in enumerate(prs.slides):
                tier, key = probe_entries[i]
                probe_map[(tier, key)] = _effective_transition_localname(slide)
        except Exception:
            probe_map = {tk: None for tk in probe_entries}

        if any(v is None for v in probe_map.values()):
            # 降级：逐个效果单独探测（打开失败/被丢弃的项）
            for tk in probe_entries:
                if probe_map[tk] is not None:
                    continue
                tier, key = tk
                single = os.path.abspath(os.path.join(workdir, "single_%s.pptx" % key))
                single_saved = os.path.abspath(
                    os.path.join(workdir, "single_saved_%s.pptx" % key))
                note = None
                try:
                    build_pptx([probe_png], single, transition=key, advance=1.0)
                    _open_save_close(single, single_saved)
                    prs2 = Presentation(single_saved)
                    probe_map[tk] = _effective_transition_localname(prs2.slides[0])
                except Exception:
                    probe_map[tk] = None
                    note = "open_failed"
                if note:
                    probe_map[tk] = (probe_map[tk], note)
        results = []
        for tier, key in entries:
            if _gated(tier):
                results.append(_result(tier, key, None, note=_gated_note(tier)))
            else:
                observed = probe_map.get((tier, key))
                note = None
...
def export_mp4(pptx_path, mp4_path, resolution=720, fps=30, quality=85,
    ...
    try:
        app = _new_powerpoint()
        # PowerPoint 2007 及以下没有 CreateVideo，提前快速失败，避免干等到超时
        try:
            _ver_major = int(str(app.Version).split(".")[0])
        except Exception:
            _ver_major = 0
        if _ver_major < 14:
            raise SkillError(
                f"本机 PowerPoint 版本 {app.Version} 不支持导出 MP4（需 PowerPoint 2010+）。"
                "可改用 build 生成 PPTX 后手动导出。", 3)
        try:
...
def cmd_build(args):
    ...
    output = args.output or default_output_path("pptx")
    guard_output_path(output, "PPTX")
    info = build_pptx(images, output, music=args.music,
...
def cmd_set_transitions(args):
    if not args.transition and not args.transitions_json and args.advance is None:
        raise SkillError("需要提供 --transition / --transitions-json / --advance 之一")
    per_slide = parse_transitions_json(args.transitions_json) if args.transitions_json else None
    ...
    for i, slide in enumerate(prs.slides):
        spec = specs[i]
        # CLI 级 --advance 补进未由 JSON 逐页指定的页（advance_ms="keep" 表示沿用原时长）
        if spec.get("advance") is None and advance_ms != "keep":
            spec["advance"] = args.advance
        if not spec.get("transition"):
            # 只改换片时间：保留已有切换效果，原地更新 advTm
            if spec.get("advance") is not None:
                set_advance_only(slide, int(round(float(spec["advance"]) * 1000)))
                count += 1
            continue
        ms = advance_ms
        if spec.get("advance") is not None:
...
def cmd_export(args):
    output = args.output
    if not output:
        stem = os.path.splitext(os.path.basename(args.pptx))[0] or "video"
        output = os.path.join(default_output_root(), stem + ".mp4")
        _ensure_parent(output)
    guard_output_path(output, "视频")
    info = export_mp4(args.pptx, output, resolution=args.resolution,
...
def cmd_all(args):
    images = collect_images(args.images, recursive=args.recursive)
    per_slide = parse_transitions_json(args.transitions_json) if args.transitions_json else None
    output_mp4 = os.path.abspath(args.output) if args.output else default_output_path("mp4")
    guard_output_path(output_mp4, "视频")
    pptx_path = os.path.abspath(args.pptx) if args.pptx else \
        os.path.splitext(output_mp4)[0] + ".pptx"
    guard_output_path(pptx_path, "PPTX")
...
def _build_parser():
    ...
    pb.add_argument("--output", default=None, metavar="文件",
                    help="输出 .pptx 路径（缺省写入默认产物目录，见 PHOTOS2MP4_OUT_DIR）")
    ...
    pe.add_argument("--output", default=None, metavar="文件",
                    help="输出 .mp4 路径（缺省为 <默认产物目录>/<pptx同名>.mp4）")
    ...
    pa.add_argument("--output", default=None, metavar="文件",
                    help="输出 .mp4 路径（缺省写入默认产物目录，见 PHOTOS2MP4_OUT_DIR）")
